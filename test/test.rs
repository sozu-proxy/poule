...
pub fn test_checkout_checkin() {
    let mut pool: Pool<Dirty<i32>> = Pool::with_capacity(10, 10, || Dirty(0));
...
pub fn test_multiple_checkouts() {
    let mut pool: Pool<i32> = Pool::with_capacity(10, 10, || 0);
...
pub fn test_depleting_pool() {
    let mut pool: Pool<i32> = Pool::with_capacity(5, 5, || 0);
...
pub fn test_resetting_pool() {
    let mut pool: Pool<Vec<i32>> = Pool::with_capacity(1, 1, || Vec::new());
    {
        let mut val = pool.checkout().unwrap();
        val.push(5);
        val.push(6);
...
#[test]
pub fn test_growing() {
    let mut pool: Pool<i32> = Pool::with_capacity(0, 10, || 0);
...
pub fn test_works_with_drop_types() {
    let _ = poule::Pool::with_capacity(1, 1, || Zomg);
}

#[test]
#[should_panic]
pub fn test_safe_when_init_panics() {
    let _ = poule::Pool::<Zomg>::with_capacity(1, 1, || panic!("oops"));
}
